...
export class Roulette extends EventTarget {
  ...
  private _lastDebugPublish: number = 0;
  private _simulationElapsedMs: number = 0;
  ...
  private _raceStartedAt: number | null = null;
  ...
  private _update() {
    ...
    while (this._elapsed >= this._updateInterval) {
      ...
      if (this._raceStartedAt !== null && this._marbles.length > 0) {
        this._simulationElapsedMs += interval * 1000;
      }
    ...
    this._render();
    if (currentTime - this._lastDebugPublish >= 250) {
      this._renderer.canvas.dataset.rouletteDebug = JSON.stringify(this.getDebugSnapshot());
      this._lastDebugPublish = currentTime;
    }
  ...
  private _updateMarbles(deltaTime: number) {
    ...
    const finishedMarbleIds = new Set<number>();
    ...
    for (let i = 0; i < this._marbles.length; i++) {
      ...
      if (marble.y > this._stage.goalY) {
        finishedMarbleIds.add(marble.id);
    ...
    this._marbles = this._marbles.filter((marble) => !finishedMarbleIds.has(marble.id) && marble.y <= goalY);
  }
  ...
  public start() {
    ...
    this._raceStartedAt = Date.now();
    this._simulationElapsedMs = 0;
    ...
    members.forEach((member) => {
      if (member) {
        for (let j = 0; j < member.count; j++) {
          ...
          this._marbles.push(
            new Marble(this.physics, order, totalCount, member.name, member.weight, this._stage?.antiStuck)
          );
        }
      }
    });
  ...
  public getDebugSnapshot() {
    return {
      ...
      elapsedMs: this._raceStartedAt === null ? 0 : Date.now() - this._raceStartedAt,
      simulationElapsedMs: Math.round(this._simulationElapsedMs),
      isRunning: this._isRunning,
      activeCount: this._marbles.length,
      totalCount: this._totalMarbleCount,
      nudgeCount: this.physics.getNudgeCount(),
      marbles: this._marbles.map((marble) => ({
        id: marble.id,
        name: marble.name,
        ...this.physics.getMarbleMotion(marble.id),
      })),
