...
export class RouletteRenderer {
  ...
  private renderWinner({ winner, theme }: RenderParameters) {
    ...
    const maxNameWidth = Math.max(220, textRightX - this._sceneCanvas.width / 2 - 24);
    const winnerNameGraphemes = Array.from(winner.name);
    const winnerDisplayName =
      winnerNameGraphemes.length > 20 ? `${winnerNameGraphemes.slice(0, 19).join('')}…` : winner.name;
    this.ctx.font = 'bold 72px sans-serif';
    const measuredNameWidth = this.ctx.measureText(winnerDisplayName).width;
    const winnerFontSize = Math.max(22, Math.min(72, (72 * maxNameWidth) / measuredNameWidth));
    this.ctx.font = `bold ${winnerFontSize}px sans-serif`;
    this.ctx.fillStyle = `hsl(${winner.hue} 100% ${theme.marbleLightness})`;
    if (theme.winnerOutline) {
      this.ctx.strokeText(
        winnerDisplayName,
        textRightX,
        this._sceneCanvas.height - 55 + WINNER_TEXT_OFFSET,
        maxNameWidth
      );
    }
    this.ctx.fillText(winnerDisplayName, textRightX, this._sceneCanvas.height - 55 + WINNER_TEXT_OFFSET, maxNameWidth);
    this.ctx.restore();
  }
}
