...
export class Marble {
  ...
  private _rescueLevel = 0;
  private _bestY = Number.NEGATIVE_INFINITY;
  ...
  update(deltaTime: number) {
    ...
    const madeProgress = this.antiStuck
      ? position.y > this._bestY + 0.12
      : Vector.lenSq(Vector.sub(this.lastPosition, position)) >= 0.00001;

    if (madeProgress) {
      this._bestY = Math.max(this._bestY, position.y);
      this._stuckTime = 0;
      this._rescueLevel = 0;
    } else if (this.isActive) {
      this._stuckTime += deltaTime;
      if (this._stuckTime > (this.antiStuck?.delayMs ?? STUCK_DELAY)) {
        this.physics.shakeMarble(this.id, this._rescueLevel);
        this._rescueLevel = Math.min(4, this._rescueLevel + 1);
        this._stuckTime = 0;
      }
    } else {
      this._stuckTime = 0;
  ...
  private _drawName(ctx: CanvasRenderingContext2D, zoom: number) {
    transformGuard(ctx, () => {
      const graphemes = Array.from(this.name);
      const displayName = graphemes.length > 8 ? `${graphemes.slice(0, 7).join('')}…` : this.name;
      const fontSize = Math.max(9, 12 - Math.max(0, graphemes.length - 4) * 0.65);
      ...
      // Stagger adjacent labels so classroom-sized names remain legible at the starting line.
      ctx.translate(this.x, this.y + (this.id % 2 === 0 ? 0.34 : -0.34));
